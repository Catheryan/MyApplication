...
package com.yzh.itteam.db.table;

import android.text.TextUtils;

import com.lidroid.xutils.db.annotation.Id;
import com.lidroid.xutils.db.annotation.Table;
import com.yzh.itteam.db.converter.ColumnConverterFactory;
import com.yzh.itteam.util.LogUtils;
...
public class TableUtils {
	...
	private static void addColumns2Map(Class<?> entityType,
			String primaryKeyFieldName, HashMap<String, Column> columnMap) {
		...
		try {
			...
			for (Field field : fields) {
				...
				if (ColumnConverterFactory.isSupportColumnConverter(field
						.getType())) {
			...
			if (!Object.class.equals(entityType.getSuperclass())) {
				addColumns2Map(entityType.getSuperclass(), primaryKeyFieldName,
						columnMap);
			}
		} catch (Throwable e) {
	 ...
	 * key: entityType.name
	 */
	private static ConcurrentHashMap<String, com.yzh.itteam.db.table.Id> entityIdMap = new ConcurrentHashMap<String, com.yzh.itteam.db.table.Id>();

	/* package */
	static synchronized com.yzh.itteam.db.table.Id getId(Class<?> entityType) {
		if (Object.class.equals(entityType)) {
		...
		com.yzh.itteam.db.table.Id id = new com.yzh.itteam.db.table.Id(entityType, primaryKeyField);
		entityIdMap.put(entityType.getName(), id);
		return id;
	}

	private static String getPrimaryKeyFieldName(Class<?> entityType) {
		com.yzh.itteam.db.table.Id id = getId(entityType);
