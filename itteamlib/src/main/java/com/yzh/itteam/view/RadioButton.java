	...
	private void init(Context context, AttributeSet attrs, int defStyle) {
		...
		if (array != null) {
			...
			for (int i = 0; i < count; i++) {
				...
				if(index == R.styleable.TabTextView_bottom_height){
					bottomHeight = array.getDimensionPixelSize(index, -1);
				}
				if(index == R.styleable.TabTextView_bottom_width){
					bottomWidth = array.getDimensionPixelSize(index, -1);
				}

				if(index == R.styleable.TabTextView_left_height){
					leftHeight = array.getDimensionPixelSize(index, -1);
				}
				if(index == R.styleable.TabTextView_left_width){
					leftWidth = array.getDimensionPixelSize(index, -1);
				}

				if(index == R.styleable.TabTextView_right_height){
					rightHeight = array.getDimensionPixelSize(index, -1);
				}
				if(index == R.styleable.TabTextView_right_width2){
					rightWidth = array.getDimensionPixelSize(index, -1);
				}

				if(index == R.styleable.TabTextView_top_height){
					topHeight = array.getDimensionPixelSize(index, -1);
				}
				if(index == R.styleable.TabTextView_top_width){
