...
import com.yzh.itteam.MApplication;
...
public class ToolLocation {
	...
	public static LocationClient mLocClient = new LocationClient(MApplication.gainContext());
	...
	private static LocationClientOption configOptions() {
		...
		if (isGpsOPen()) {
			...
			if (ToolNetwork.getInstance().init(MApplication.gainContext())
					.isConnected()) {
